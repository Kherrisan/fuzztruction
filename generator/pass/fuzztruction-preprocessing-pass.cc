...
#define DEBUG_TYPE "ft-preprocessing"
...
compiler wrapper.
*/
PreservedAnalyses FuzztructionSourcePreprocesssingPass::run(Module &M, ModuleAnalysisManager &)
{
    LLVM_DEBUG(dbgs() << "Running FuzztructionSourcePreprocesssingPass\n");
...
bool FuzztructionSourcePreprocesssingPass::replaceMemFunctions(Module &M)
{
    ...
    LLVM_DEBUG(dbgs() << "FT DEBUG: Module name " << M.getName() << "\n");
    ...
    for (auto &F : *IM)
    {
        ...
        if (!orig_f)
        {
            LLVM_DEBUG(dbgs() << "FT DEBUG: " << orig_name << " not used in " << M.getName() << "\n");
            continue;
        }
        LLVM_DEBUG(dbgs() << "FT DEBUG: Importing " << new_name << "\n");
        ...
        for (const Argument &I : F.args())
            ...
            { // Is this argument preserved?
                LLVM_DEBUG(dbgs() << "FT DEBUG: arg=" << I.getName() << "\n");
        ...
        LLVM_DEBUG(dbgs() << "FT DEBUG: Replacing " << orig_f->getName().str() << " by " << new_name << "\n");
